...
def sigma(dists, kth=8):
    ...
    try:
        ...
        # this sigma value is irrelevant since not used for final
        # compute_edge_list
...
def compute_edges_list(A, kth=8 + 1):
    ...
    if num_nodes > 9:
        ...
        knn_values = np.partition(
            A, new_kth - 1, axis=-1)[:, new_kth:-1]  # NEW
    else:
        ...
        if num_nodes != 1:
            knn_values = A[knns != np.arange(num_nodes)[:, None]].reshape(
                num_nodes, -1)  # NEW
            knns = knns[knns != np.arange(
                num_nodes)[:, None]].reshape(num_nodes, -1)
    ...
    def _prepare(self):
        ...
        for index, sample in enumerate(self.sp_data):
            ...
            if self.use_mean_px:
                A = compute_adjacency_matrix_images(
                    coord, mean_px)  # using super-pixel locations + features
            else:
                A = compute_adjacency_matrix_images(
                    coord, mean_px, False)  # using only super-pixel locations
        ...
        for index in range(len(self.sp_data)):
            ...
            for src, dsts in enumerate(self.edges_lists[index]):
                ...
                # since, VOC Superpixels has few samples (5 samples) with only
                # 1 node
            ...
            # dim same as node feature dim
            edge_feat_dim = g.ndata['feat'].shape[1]
            #g.edata['feat'] = torch.ones(g.number_of_edges(), edge_feat_dim).half()
            g.edata['feat'] = torch.Tensor(
                self.edge_features[index]).unsqueeze(1).half()  # NEW
    ...
    def collate(self, samples):
        ...
        tab_snorm_e = [
            torch.FloatTensor(
                ...
                1).fill_(
                1. /
                float(size)) for size in tab_sizes_e]
